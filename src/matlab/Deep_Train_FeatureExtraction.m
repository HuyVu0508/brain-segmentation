function [feature,index] = Deep_FeatureExtraction_WholeBrain(Mask,RawData,featureopen,b,typeoffeature)
%     Input:
    %     RawData (128*256*256): RawData image of a brain MRI
...
%     Output:
    ...
    %     of chosen voxel
    %     index (number of chosen voxel * 3) : toa x,y,z cua cac chosen voxel trong RawData

    % Feature
    [x,y,z] = ind2sub(size(RawData),find(Mask));    
    si = featureopen*2 + 1;
    if(typeoffeature==1)
    ...
    meangrey = mean(RawData(RawData>0));
    RawDataNormalized = double(RawData)*100/meangrey;
    RawDataNormalized = (double(RawDataNormalized)-100)/200;
%     figure;
%     histogram(RawDataNormalized(RawData>0));
%     xlim([1,150]);
    
    
    % Normalize toa do
    brainregion = RawData > 0;
    [x0,y0,z0] = ind2sub(size(RawData),find(brainregion));  
...
%         end
        ...
        feature(i,:) = FeatureExtraction1pix(x(i),y(i),z(i),rangex,rangey,rangez,RawDataNormalized,featureopen,b,typeoffeature);
    end
end

function [feature1pix] = FeatureExtraction1pix(x,y,z,rangexnorm,rangeynorm,rangeznorm,RawData,featureopen,b,typeoffeature)
    rangex = [1 size(RawData,1)];
    ...
    if(typeoffeature==1)
        si = featureopen*2 + 1;
        z1slices = RawData(max(rangex(1),x-featureopen):min(rangex(2),x+featureopen),y-featureopen:y+featureopen,z-1:z+1);
    ...
    % Type of feature 2    
        ...
        si = featureopen*2 + 1;        
        x3slices = RawData(max(rangex(1),x-1):min(rangex(2),x+1),y-featureopen:y+featureopen,z-featureopen:z+featureopen);
        if(rangex(1) > x-1)
        ...
        y3slices = RawData(max(rangex(1),x-featureopen):min(rangex(2),x+featureopen),y-1:y+1,z-featureopen:z+featureopen);             
        if(rangex(1) > x-featureopen)
        ...
        z3slices = RawData(max(rangex(1),x-featureopen):min(rangex(2),x+featureopen),y-featureopen:y+featureopen,z-1:z+1);
        if(rangex(1) > x-featureopen)
            phan_bu = zeros(rangex(1)-(x-featureopen),si,3);
